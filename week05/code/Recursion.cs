...
public static class Recursion
{
    ...
    public static void PermutationsChoose(List<string> results, string letters, int size, string word = "")
    {
        ...
        for (int i = 0; i < letters.Length; i++)
        {
            ...
            string chosenLetter = letters[i].ToString();

            // Remove that letter from the remaining letters
            string remainingLetters = letters[..i] + letters[(i + 1)..];

            // Recurse with the new word
    ...
    {
        // Initialize dictionary if it's the first call
        if (remember == null)
        {
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        // Base case:
        // If there are no more wildcards, this is a complete binary string
        if (index == -1)
        {
            results.Add(pattern);
            return;
